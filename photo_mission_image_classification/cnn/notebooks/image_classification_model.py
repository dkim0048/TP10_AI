...
Original file is located at
    https://colab.research.google.com/drive/11MyG9gr2j7WAjiksZSZGxTXD28FjXpQ4

## **Lexical Bridge Photo Mission - Image Classification Model**

### **Purpose**

The purpose of this notebook is to train and evaluate a custom CNN model for the Lexical Bridge photo mission feature. The model classifies uploaded images into one of nine target object classes, such as elephant, giraffe, penguin, fish, books, clock, umbrella, airplane, bicycle, and ship. This supports the game flow where a child is asked to take or upload a photo of a specific object, and the system checks whether the image matches the requested class.

This notebook trains a residual CNN model from scratch using the prepared image dataset ZIP file. To improve fine-grained classification and reduce confusion between visually similar classes such as cat and dog, the model uses residual blocks, stronger data augmentation, AdamW optimization, and a cosine learning rate schedule. Training images are augmented with random resized cropping, flipping, rotation, and colour jittering, while validation and test images use only deterministic preprocessing. The best-performing checkpoint is saved for later deployment in the photo mission prototype.
...
class BaselineCNN(nn.Module):
    """
    Custom residual CNN for 10 class image classification
