# -*- coding: utf-8 -*-
"""T4_LR_Scheduler.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1okE45yfKEGhO-8l3G9MTeqIX_7VCzQDs

## **Image Classification Model - T4 LR Scheduler**

### **Purpose**

The purpose of this notebook is to train and evaluate a regularized CNN model for the Lexical Bridge Photo Mission feature. The model is trained to classify uploaded images into one of nine target object classes, such as cat, dog, bird, flower, fruit, vegetable, car, bicycle, and shoe. This supports the game flow where a child is asked to take or upload a photo of a specific object, and the system checks whether the image matches the requested class.

This trial builds on the previous higher-resolution regularized CNN model. It keeps the 224 × 224 input size, batch size of 32, lighter training augmentation, Batch Normalization, Global Average Pooling, and reduced Dropout settings. The main change is reducing the training duration from 100 epochs to 60 epochs and adding a ReduceLROnPlateau learning rate scheduler, which reduces the learning rate by 0.5 when validation loss does not improve for 5 epochs. This is used to make training more stable and reduce unnecessary extended training while still saving the checkpoint with the highest validation accuracy for final test evaluation.
...
7. Train the regularized CNN model with scheduler-based learning rate adjustment
